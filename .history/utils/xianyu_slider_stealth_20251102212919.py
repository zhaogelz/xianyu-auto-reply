...
# 4. [USER REQUEST] Reduced trajectory points and slide duration for faster movement.
# 5. [GEMINI OPTIMIZATION] Further reduced slide duration and auxiliary delays while keeping step count constant.
...
class XianyuSliderStealth:
    ...
    def generate_human_trajectory(self: Self, distance: float) -> Tuple[Any, str, float, int]:
        ...
        if cycle_position == 0:
            if cycle_number == 1:
                target_total_time = random.uniform(0.3, 0.7) # 原: random.uniform(0.5, 1.2)
                trajectory_points = random.randint(40, 70) # 保持不变 (原: 80-150)
                sliding_mode = "初次超速模式" # 原: "初次加速模式"
            else:
                target_total_time = random.uniform(0.25, 0.6) # 原: random.uniform(0.4, 1.0)
                trajectory_points = random.randint(30, 60) # 保持不变 (原: 60-120)
                sliding_mode = f"第{cycle_number}轮超速模式" + (" [失败后将刷新]" if self.Refresh else "")
        elif cycle_position == 1:
            base_speed = max(0.1, 0.3 - cycle_number * 0.1) # 原: max(0.2, 0.5 - ...)
            target_total_time = random.uniform(base_speed, base_speed + 0.2) # 原: base_speed + 0.3
            trajectory_points = random.randint(20, 40) # 保持不变 (原: 30-60)
            sliding_mode = f"第{cycle_number}轮闪电模式" # 原: "极速模式"
        else:
            target_total_time = random.uniform(0.2, 0.5) # 原: random.uniform(0.4, 0.8)
            trajectory_points = random.randint(30, 50) # 保持不变 (原: 50-90)
            sliding_mode = f"第{cycle_number}轮极速反思模式" # 原: "快速反思模式"
        # --- 结束优化 ---
    ...
    def simulate_slide(self: Self, slider_button: Element, trajectory: Any, target_total_time: float, strategy_name: str) -> Any:
        ...
        """
        # --- 修改点: 调整 "急躁" 判断标准 ---
        is_impatient = "急躁模式" in strategy_name or "极速模式" in strategy_name or "闪电模式" in strategy_name
        is_fast_mode = "加速模式" in strategy_name or "快速" in strategy_name or "超速模式" in strategy_name
        # --- 结束修改 ---
        ...
        try:
            ...
            # --- 优化点: 进一步加速 ---
            if is_impatient:
                observation_time = random.uniform(0.02, 0.1) # 原: 0.05, 0.2
            else: # (包括 is_fast_mode 和 默认)
                observation_time = random.uniform(0.1, 0.3) # 原: random.uniform(0.2, 0.5)
            # --- 结束优化 ---
            ...
            if is_impatient:
                slider_button.hover()
                time.sleep(random.uniform(0.01, 0.03)) # 原: 0.01, 0.05
                self.page.actions.hold(slider_button)
                time.sleep(random.uniform(0.01, 0.03)) # 原: 0.01, 0.05
            else: # (包括 is_fast_mode)
                slider_button.hover()
                time.sleep(random.uniform(0.02, 0.08)) # 原: 0.05, 0.15
                self.page.actions.hold(slider_button)
                time.sleep(random.uniform(0.02, 0.08)) # 原: 0.05, 0.15
            # --- 结束优化 ---
        ...
        # --- 优化点: 进一步加速 ---
        if is_impatient:
            time.sleep(random.uniform(0.01, 0.05)) # 原: 0.02, 0.1
            self.page.actions.release()
            time.sleep(random.uniform(0.02, 0.08)) # 原: 0.05, 0.15
        else:
            # 快速模式微调 (降低概率和时间)
            if random.random() < 0.3: # 保持
                adj_dist = random.uniform(-2, 3) # 保持
                try:
                    self.page.actions.move(offset_x=int(adj_dist), offset_y=0, duration=0.1) # 保持
                except Exception: pass
                time.sleep(random.uniform(0.02, 0.05)) # 原: 0.05, 0.1
            
            time.sleep(random.uniform(0.05, 0.15)) # 确认停顿 (原: 0.1, 0.3)
            self.page.actions.release()
            time.sleep(random.uniform(0.05, 0.15)) # 释放后停顿 (原: 0.1, 0.3)
        # --- 结束优化 ---
    ...
    def _simulate_page_entry(self: Self):
        ...
        try:
            ...
            # --- 修改点: 减少次数和时间 (保留) ---
            # --- 优化点: 进一步加速 ---
            for _ in range(random.randint(1, 3)): # 保持 (原: 3, 6)
                self.page.actions.move(
                    offset_x=random.randint(-50, 50),
                    offset_y=random.randint(-30, 30),
                    duration=random.uniform(0.1, 0.2) # 保持 (原: 0.15, 0.4)
                )
                time.sleep(random.uniform(0.02, 0.05)) # 原: 0.05, 0.1
            # --- 结束优化 ---
        except Exception as e:
    ...
    def _simulate_looking_for_captcha(self: Self):
        ...
        try:
            ...
            # --- 修改点: 减少次数和时间 (保留) ---
            # --- 优化点: 进一步加速 ---
            for _ in range(random.randint(1, 2)): # 保持 (原: 2, 4)
                self.page.actions.move(
                    offset_x=random.randint(-100, 100),
                    offset_y=random.randint(-80, 80),
                    duration=random.uniform(0.1, 0.25) # 保持 (原: 0.2, 0.5)
                )
                time.sleep(random.uniform(0.05, 0.15)) # 原: 0.1, 0.3
            # --- 结束优化 ---
        except Exception as e:
        ...
        try:
            ...
            for _ in range(random.randint(1, 2)): # 保持 (原: 2, 4)
                self.page.actions.move(
                    ...
                    offset_y=random.randint(-30, 30),
                    duration=random.uniform(0.1, 0.2) # 保持 (原: 0.15, 0.35)
                )
                time.sleep(random.uniform(0.02, 0.08)) # 原: 0.05, 0.15
            # --- 结束优化 ---
        except Exception as e:
    ...
    def _simulate_post_verification_activity(self: Self):
        ...
        try:
            ...
            # --- 修改点: 减少次数和时间 (保留) ---
            # --- 优化点: 进一步加速 ---
            for _ in range(random.randint(1, 2)): # 保持 (原: 2, 3)
                self.page.actions.move(
                    offset_x=random.randint(-200, 200),
                    offset_y=random.randint(-100, 100),
                    duration=random.uniform(0.1, 0.3) # 保持 (原: 0.3, 0.6)
                )
                time.sleep(random.uniform(0.05, 0.1)) # 原: 0.1, 0.2
            # --- 结束优化 ---
        except Exception as e:
