...
class XianyuSliderStealth:
    ...
    def generate_human_trajectory(self: Self, distance: float) -> Tuple[Any, str, float, int]:
        ...
        if cycle_position == 0:
            if cycle_number == 1:
                target_total_time = random.uniform(0.5, 1.2) # 原: random.uniform(2.0, 4.0)
                trajectory_points = random.randint(40, 70) # 原: random.randint(80, 150)
                sliding_mode = "初次加速模式" # 原: "初次谨慎模式"
            else:
                target_total_time = random.uniform(0.4, 1.0) # 原: random.uniform(1.5, 3.0)
                trajectory_points = random.randint(30, 60) # 原: random.randint(60, 120)
                sliding_mode = f"第{cycle_number}轮加速模式" + (" [失败后将刷新]" if self.Refresh else "")
        elif cycle_position == 1:
            base_speed = max(0.2, 0.5 - cycle_number * 0.1) # 原: max(0.2, 1.0 - cycle_number * 0.1)
            target_total_time = random.uniform(base_speed, base_speed + 0.3) # 原: base_speed + 0.4
            trajectory_points = random.randint(20, 40) # 原: random.randint(30, 60)
            sliding_mode = f"第{cycle_number}轮极速模式" # 原: "急躁模式"
        else:
            target_total_time = random.uniform(0.4, 0.8) # 原: random.uniform(1.0, 2.0)
            trajectory_points = random.randint(30, 50) # 原: random.randint(50, 90)
            sliding_mode = f"第{cycle_number}轮快速反思模式" # 原: "反思模式"
        # --- 结束修改 ---
    ...
    def simulate_slide(self: Self, slider_button: Element, trajectory: Any, target_total_time: float, strategy_name: str) -> Any:
        ...
        """
        # --- 修改点: 调整 "急躁" 判断标准 ---
        is_impatient = "急躁模式" in strategy_name or "极速模式" in strategy_name
        is_fast_mode = "加速模式" in strategy_name or "快速" in strategy_name
        # --- 结束修改 ---
        ...
        try:
            ...
            # --- 修改点: 缩短观察时间 ---
            if is_impatient:
                observation_time = random.uniform(0.05, 0.2) # 原: 0.1, 0.5
            else: # (包括 is_fast_mode 和 默认)
                observation_time = random.uniform(0.2, 0.5) # 原: random.uniform(0.8, 2.5)
            # --- 结束修改 ---
            ...
            if is_impatient:
                slider_button.hover()
                time.sleep(random.uniform(0.01, 0.05)) # 原: 0.02, 0.08
                ...
                self.page.actions.hold(slider_button)
                time.sleep(random.uniform(0.05, 0.15)) # 原: 0.1, 0.4
            # --- 结束修改 ---
        ...
        # --- 修改点: 缩短释放延迟 ---
        if is_impatient:
            time.sleep(random.uniform(0.02, 0.1)) # 原: 0.05, 0.2
            self.page.actions.release()
            time.sleep(random.uniform(0.05, 0.15)) # 原: 0.1, 0.3
        else:
            # 快速模式微调 (降低概率和时间)
            if random.random() < 0.3: # 原: 0.6
                adj_dist = random.uniform(-2, 3) # 原: -3, 5
                try:
                    self.page.actions.move(offset_x=int(adj_dist), offset_y=0, duration=0.1) # 原: 0.15
                except Exception: pass
                time.sleep(random.uniform(0.05, 0.1)) # 原: 0.1, 0.25
            
            time.sleep(random.uniform(0.1, 0.3)) # 确认停顿 (原: 0.2, 0.8)
            self.page.actions.release()
            time.sleep(random.uniform(0.1, 0.3)) # 释放后停顿 (原: 0.3, 0.8)
        # --- 结束修改 ---
    ...
    def _simulate_page_entry(self: Self):
        ...
        try:
            ...
            # --- 修改点: 减少次数和时间 ---
            for _ in range(random.randint(1, 3)): # 原: 3, 6
                self.page.actions.move(
                    offset_x=random.randint(-50, 50),
                    offset_y=random.randint(-30, 30),
                    duration=random.uniform(0.1, 0.2) # 原: 0.15, 0.4
                )
                time.sleep(random.uniform(0.05, 0.1)) # 原: 0.1, 0.25
            # --- 结束修改 ---
        except Exception as e:
    ...
    def _simulate_looking_for_captcha(self: Self):
        ...
        try:
            ...
            # --- 修改点: 减少次数和时间 ---
            for _ in range(random.randint(1, 2)): # 原: 2, 4
                self.page.actions.move(
                    offset_x=random.randint(-100, 100),
                    offset_y=random.randint(-80, 80),
                    duration=random.uniform(0.1, 0.25) # 原: 0.2, 0.5
                )
                time.sleep(random.uniform(0.1, 0.3)) # 原: 0.3, 0.8
            # --- 结束修改 ---
        except Exception as e:
        ...
        try:
            ...
            for _ in range(random.randint(1, 2)): # 原: 2, 4
                self.page.actions.move(
                    ...
                    offset_y=random.randint(-30, 30),
                    duration=random.uniform(0.1, 0.2) # 原: 0.15, 0.35
                )
                time.sleep(random.uniform(0.05, 0.15)) # 原: 0.1, 0.3
            # --- 结束修改 ---
        except Exception as e:
    ...
    def _simulate_post_verification_activity(self: Self):
        ...
        try:
            ...
            # --- 修改点: 减少次数和时间 ---
            for _ in range(random.randint(1, 2)): # 原: 2, 3
                self.page.actions.move(
                    offset_x=random.randint(-200, 200),
                    offset_y=random.randint(-100, 100),
                    duration=random.uniform(0.1, 0.3) # 原: 0.3, 0.6
                )
                time.sleep(random.uniform(0.1, 0.2)) # 原: 0.2, 0.5
            # --- 结束修改 ---
        except Exception as e:
